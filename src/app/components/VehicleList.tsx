type VehicleModel = {
  ...
  export default function VehicleList({ models }: VehicleListProps) {
    if (!models) {
      return <p className="text-center text-gray-600">Loading vehicle models...</p>;
    }
  
    if (models.length === 0) {
      return <p className="text-center text-gray-600">No vehicle models found for the selected criteria.</p>;
    }
  
    return (
      <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {models.map((model) => (
          <li key={model.Model_ID} className="bg-white shadow rounded-lg p-4 hover:shadow-lg transition-shadow">
